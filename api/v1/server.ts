...
const app = new Hono();
const config = getServerConfig('v1');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(`MAS API Server v1 starting on ${config.host}:${config.port}`);
  ...
  serve({
    ...
    console.log(`API v1 is running at http://${info.address}:${info.port}`);
  });
}