...
const DocumentContent: React.FC<DocumentContentProps> = ({
  ...
  const renderFileTree = (items: DocumentFile[], level = 0, onSelect?: () => void) => {
    return items.map((item) => (
      <div key={item.path} style={{ marginLeft: `${level * 12}px` }}>
        {item.type === 'directory' ? (
          <>
            <div className="py-1 font-medium text-mas-text-secondary text-xs">
              {item.name}/
            </div>
          ...
          <div
            className={`
              py-1 px-1.5 cursor-pointer rounded transition-colors text-xs
  ...
  return (
    <div className="h-full flex flex-col">
      {/* File List Toggle Button - visible below 2xl */}
      {documents.length > 0 && (
        <button
          onClick={() => setFileListOpen(!fileListOpen)}
          className="2xl:hidden fixed bottom-4 right-4 z-20 p-3 bg-mas-blue text-mas-bg-root rounded-full shadow-lg hover:bg-mas-blue-soft transition-colors"
        >
      ...
      {fileListOpen && (
        <div
          className="2xl:hidden fixed inset-0 bg-black/50 z-10"
          onClick={() => setFileListOpen(false)}
        />
      )}

      {/* Mobile/Tablet File List - slide-in from right */}
      <div className={`
        2xl:hidden fixed inset-y-0 right-0 z-20
        w-64 border-l border-mas-border overflow-y-auto bg-mas-bg-panel
        ...
        <div className="p-3">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <FolderOpen size={16} className="text-mas-text-muted" />
              <span className="text-sm font-medium text-mas-text">Files</span>
            </div>
            <button
              onClick={() => setFileListOpen(false)}
          ...
          <div className="text-xs font-mono text-mas-text-muted mb-2 truncate">
            {agentId}/openspec/
          </div>
          <div className="border-t border-mas-border pt-2">
            {documents.length > 0 ? (
              renderFileTree(documents, 0, () => setFileListOpen(false))
            ) : (
              <div className="text-mas-text-muted text-xs">No documents found</div>
            )}
          </div>
        </div>
      </div>

      {/* Desktop Layout - File List as collapsible header section */}
      <div className="hidden 2xl:block border-b border-mas-border bg-mas-bg-panel flex-shrink-0">
        <div className="px-4 py-2">
          <div className="flex items-center gap-2 mb-2">
            <FolderOpen size={14} className="text-mas-text-muted" />
            <span className="text-xs font-medium text-mas-text-secondary uppercase tracking-wide">File Browser</span>
            <span className="text-xs text-mas-text-muted font-mono">— {agentId}/openspec/</span>
          </div>
          <div className="flex flex-wrap gap-1">
            {documents.length > 0 ? (
              renderFlatFileList(documents, selectedDocument, onSelectDocument)
            ) : (
              <div className="text-mas-text-muted text-xs py-1">No documents found</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 sm:p-6 pb-20 2xl:pb-6">
          {loading ? (
            ...
            <>
              {/* Document Header */}
              {selectedDocument && (
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-mas-border">
                  <FileText size={16} className="text-mas-blue" />
                  <span className="text-sm font-medium text-mas-text">{selectedDocument}</span>
                </div>
              )}
...
function renderFlatFileList(
  ...
  for (const item of items) {
    if (item.type === 'directory') {
      if (item.children) {
        result.push(...renderFlatFileList(item.children, selectedDocument, onSelectDocument, `${prefix}${item.name}/`));
      }
    } else {
      ...
      result.push(
        <button
          ...
          className={`
            ...
            ${selectedDocument === item.path
              ? 'bg-mas-blue text-mas-bg-root border-mas-blue'
              : 'bg-mas-bg-subtle text-mas-text-secondary border-mas-border hover:bg-mas-bg-root hover:text-mas-text hover:border-mas-text-muted'}
          `}
        >
          {displayName}
        </button>
