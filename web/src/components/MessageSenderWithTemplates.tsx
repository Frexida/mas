...
指示を出して報告までさせてください。`,
    ...
    comprehensiveTaskAllocation: {
      name: "包括的タスク配分（詳細版）",
      template: `【包括的作業指示】

このメッセージを受け取ったら、以下のタスクを各ユニットに適切に割り振り、実行から報告まで完全に管理してください。
...
これらを確実に実行し、完了まで管理してください。`,
    },
    parallelExecution: {
      name: "並列実行指示",
      template: `【並列実行指示】

以下のタスクを全ユニットで同時並行で実行せよ。
...
このメッセージを受け取ったら、以下のサイクルを完全に実行してください：
...
全フェーズを確実に実行し、最終報告まで完了させてください。`,
    },
    criticalDecision: {
      name: "重要意思決定要求",
      template: `【重要意思決定要求】

以下の重要事項について、即座に意思決定が必要です：
